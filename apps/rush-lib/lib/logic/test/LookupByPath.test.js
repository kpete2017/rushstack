...
const LookupByPath_1 = require("../LookupByPath");
describe('iteratePathSegments', () => {
    it('returns empty for an empty string', () => {
        const result = [...LookupByPath_1.LookupByPath.iteratePathSegments('')];
        expect(result.length).toEqual(0);
    });
    it('returns the only segment of a trival string', () => {
        const result = [...LookupByPath_1.LookupByPath.iteratePathSegments('foo')];
        expect(result).toEqual(['foo']);
    });
    it('treats backslashes as ordinary characters, per POSIX', () => {
        const result = [...LookupByPath_1.LookupByPath.iteratePathSegments('foo\\bar\\baz')];
...
describe('findChildPath', () => {
    it('returns empty for an empty tree', () => {
        expect(new LookupByPath_1.LookupByPath().findChildPath('foo')).toEqual(undefined);
    });
    it('returns the matching node for a trivial tree', () => {
        expect(new LookupByPath_1.LookupByPath([['foo', 1]]).findChildPath('foo')).toEqual(1);
    });
    it('returns the matching node for a single-layer tree', () => {
        const tree = new LookupByPath_1.LookupByPath([
            ['foo', 1],
            ['bar', 2],
            ['baz', 3]
        ]);
...
//# sourceMappingURL=LookupByPath.test.js.map