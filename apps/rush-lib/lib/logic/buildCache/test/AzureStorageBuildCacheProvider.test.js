...
const AzureStorageBuildCacheProvider_1 = require("../AzureStorageBuildCacheProvider");
describe('AzureStorageBuildCacheProvider', () => {
    ...
    it('uses a correct list of Azure authority hosts', async () => {
        await expect(() => new AzureStorageBuildCacheProvider_1.AzureStorageBuildCacheProvider({
            storageAccountName: 'storage-account',
            storageContainerName: 'container-name',
            azureEnvironment: 'INCORRECT_AZURE_ENVIRONMENT',
    ...
    describe('isCacheWriteAllowed', () => {
        function prepareSubject(optionValue, envVarValue) {
            ...
            return new AzureStorageBuildCacheProvider_1.AzureStorageBuildCacheProvider({
                storageAccountName: 'storage-account',
                storageContainerName: 'container-name',
    ...
    async function testCredentialCache(isCacheWriteAllowed) {
        const cacheProvider = new AzureStorageBuildCacheProvider_1.AzureStorageBuildCacheProvider({
            storageAccountName: 'storage-account',
            storageContainerName: 'container-name',
            isCacheWriteAllowed
        });
...
//# sourceMappingURL=AzureStorageBuildCacheProvider.test.js.map